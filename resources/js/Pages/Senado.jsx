...
export default function Senado({ municipio, resultados = [], porPartido = [], votosLista = [], allMunicipios = [] }) {
    ...
    return (
        <AppLayout title="Senado" breadcrumb={[{ label: 'SANTANDER', href: '/' }, { label: 'SENADO' }]}>
            <Head title={`Senado ${munLabel ?? 'Santander'} — Inteligencia Electoral`} />
            ...
            <div className="bg-[var(--color-primary)] text-white px-6 py-3">
                <div className="flex items-center gap-2">
                    ...
                    <GeoNavigator municipios={allMunicipios} currentMunicipio={municipio?.name} basePath="/senado" />
                </div>
            ...
            <div className="flex flex-col xl:flex-row min-h-[60vh]">
                {/* Izquierda: candidatos nominales */}
                ...
                {/* Derecha: tabla votos por lista/partido */}
                    ...
                    {votosLista.length > 0 && (
                        <div>
                            <div className="px-5 py-3 bg-[var(--color-primary-dark)] text-white">
                                ...
                                <p className="text-[10px] text-white/50 mt-0.5">{municipio?.name ?? 'Santander'} · {fmt(totalListaVotos)} votos totales</p>
                            </div>
                            {votosLista.map((p, i) => {
