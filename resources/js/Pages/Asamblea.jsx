    ...
    return (
        <Link href={`/persona/${c.id}`} className="block px-5 py-5 border-b border-[var(--color-line)] hover:bg-blue-50/20 transition-colors group">
            ...
            <p className="text-[13px] text-[var(--color-ink-soft)] mt-1 flex items-center gap-2">
                ...
                {c.acronym ?? c.partido ?? 'Sin partido'}
            </p>
...
export default function Asamblea({ municipio, resultados = [], porPartido = [], allMunicipios = [] }) {
    ...
    const donutData = resultados.filter(c => c.votos > 0).map((c, i) => ({
    ...
    return (
        <AppLayout title="Asamblea" breadcrumb={[{ label: 'SANTANDER', href: '/' }, { label: 'ASAMBLEA' }]}>
            <Head title={`Asamblea ${munLabel ?? 'Santander'} — Inteligencia Electoral`} />
            ...
            <div className="bg-[var(--color-primary)] text-white px-6 py-3">
                <div className="flex items-center gap-2">
                    ...
                    <GeoNavigator municipios={allMunicipios} currentMunicipio={municipio?.name} basePath="/asamblea" />
                </div>
                    ...
                    {topCandidato && (
                        <div className="bg-white rounded-lg border border-[var(--color-line)] overflow-hidden mb-5">
                            ...
                            <div className="p-5">
                                ...
                                <p className="text-[11px] text-[var(--color-ink-faint)] mt-1">{topCandidato.partido ?? '—'}</p>
