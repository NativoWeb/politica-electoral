...
export default function Partidos({ partidos }) {
    ...
    function handleForceDelete(p) {
        if (!confirm(`¿ELIMINAR PERMANENTEMENTE "${p.name}"?\n\nSe borrarán todos los endorsements y se quitará el partido de los líderes. Esta accion NO se puede deshacer.`)) return;
        router.delete(`/admin/partidos/${p.id}/force`);
    }
                    ...
                    <div key={p.id} className={`bg-white border border-[var(--color-line)] rounded-xl overflow-hidden ${!p.active ? 'opacity-50' : ''}`}>
                        {editId === p.id ? (
                            ...
                            <div className="flex items-center gap-3 px-4 py-3.5">
                                ...
                                <button onClick={() => handleForceDelete(p)} className="w-9 h-9 rounded-lg bg-red-50 text-red-500 flex items-center justify-center active:bg-red-100 flex-shrink-0" title="Eliminar permanente">
